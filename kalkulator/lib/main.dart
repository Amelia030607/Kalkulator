...
class Calculator extends StatefulWidget {
  const Calculator({super.key});//class calculator adalah widget stateful karena memiliki nilai yang selalu berubah ketika tombolnya ditekan
...
class _CalculatorState extends State {
  String output = "0"; //output merupakan variable yang menampung nilaiyang akan ditampilkan di kalkulator
  ...
  String evaluateExpression(String expression) { //fungsi ini digunakan untuk mengonversi expression menjadi hasil perhitungan. fungsi ini diambil dari expression.dart
    ...
    final result = evaluator.eval(parsedExpression,{}); //digunakan untuk mengevaluasi expression dan mengambil nilai sebagai String
    return result.toString(); //mengembalikan nilai kepada String
  }

  Widget buildButton(String buttonText, Color color,
      {double widthFactor = 1.0}) { //fungsi ini digunakan untuk membuat widget tombol dengan text, warna dan ukuran yang sudah ditentukan
      ...
      child: Padding(
        ...
        child: ElevatedButton( //digunakan untuk membuat tombol dengan gaya tertentu
          style: ElevatedButton.styleFrom(
            padding: const EdgeInsets.symmetric(vertical:22), //digunakan untuk mengatur jarak dalam tombol secara vertical menjadi 22 sehingga tombol terlihat tinggi
  ...
  Widget build(BuildContext context) { //fungsi build digunakan untuk membuat tampilan utama
    return Scaffold(
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch, //tanpa CrossAxisAlignmentnstretch,ukuran tombol hanya sesuai dengan isinya.
        children: [
          Expanded(
              child: Container(
